...
public class GetWeatherByLatLng {
    ...
    @Autowired
    private ConvertGPSService convertGPSService;

    @Retryable(value = {Exception.class}, maxAttempts = 3, backoff = @Backoff(delay = 2500))
    public WeatherModel getInfo(LocationModel locationModel) throws Exception {
        ...
        GridXYModel gridXYmodel = convertGPSService.convertXY(locationModel);
    ...
    @Recover
    public WeatherModel exceptionRecover(Exception e, LocationModel locationModel) {
        WeatherModel weatherModel = new WeatherModel();
